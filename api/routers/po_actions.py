...
@router.get("/{id}/calculate-net-sales", response_model=ResponseMsg)
async def calculate_net_sales(id: int, background_tasks: BackgroundTasks):
  po_crud.update_purchase_order(
    id=id, updates=UpdatePurchaseOrder(status="Calculating Net Sales")
  )
  po_crud.add_log_to_purchase_order(
    id=id, log=Log(user="pending", message="Submitted to Calculate Net Sales.", type="user")
  )

  background_tasks.add_task(validate_for_net_sales, po_id=id)
